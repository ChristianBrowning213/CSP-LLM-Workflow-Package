...
def test_benchmark_mode_rejects_malformed_phase1_objective_payload(workdir: Path) -> None:
    ...
    case_file = _write_case_file(
        workdir / "strict_malformed_objective_cases.json",
        property_bias="qlip.native.density_packing_proxy",
    )
    payload = json.loads(case_file.read_text(encoding="utf-8"))
    payload["cases"][0]["qlip_objective"] = {"type": "density_packing", "proxy": "physical_density"}
    case_file.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    with pytest.raises(ValueError, match="PHASE1_BENCHMARK_REJECTED_MALFORMED_QLIP_OBJECTIVE"):
        run_benchmark_cases(
            case_file=case_file,
            mode="stub",
            workspace=workdir,
