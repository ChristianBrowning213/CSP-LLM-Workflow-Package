...
def test_phase1_threshold_tradeoff_end_to_end_strict_runs_truthfully(workdir: Path, monkeypatch) -> None:
    ...
    run = run_csp_pipeline(
        query="TiO2 prioritize high weighted tradeoff",
        with_spp=True,
        mode="stub",
        workspace=workdir,
        settings=settings,
    ...
    assert request["problem"]["objective"]["type"] == "threshold_tradeoff"
    assert request["problem"]["objective"]["threshold"] == {"operator": ">=", "value": 0.0}
