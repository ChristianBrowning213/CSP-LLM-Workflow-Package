...
def test_phase1_density_packing_proxy_end_to_end_strict_runs_truthfully(workdir: Path, monkeypatch) -> None:
    ...
    run = run_csp_pipeline(
        query="TiO2 prioritize high density",
        with_spp=True,
        mode="stub",
        workspace=workdir,
        settings=settings,
    ...
    assert request["problem"]["objective"] == {
