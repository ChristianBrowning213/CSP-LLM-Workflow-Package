...
def test_phase1_linear_property_proxy_strict_benchmark_mode_accepts(workdir: Path) -> None:
    settings = Settings.from_sources(None)
    settings.workspace_root = workdir
    settings.allowed_read_roots = [workdir.resolve(), Path.cwd().resolve()]
    settings.allowed_write_roots = [workdir.resolve()]
    case_file = workdir / "linear_cases.json"
                ...
                "cases": [
                    {
                        ...
                        "property_bias": "qlip.native.linear_property_proxy",
                        "qlip_objective": {
    ...
    assert report["per_case_table"][0]["qlip_objective_family"] == "linear_property"
