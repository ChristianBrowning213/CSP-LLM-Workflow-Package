...
def build_parser() -> argparse.ArgumentParser:
    parser = ExitCodeArgumentParser(description="Run the crystal system from plain terminal text.")
    parser.add_argument("text", nargs="*", help="Text request. If omitted, stdin is used.")
    parser.add_argument("--out-root", default="local_runs", help="Directory for run outputs.")
    parser.add_argument("--run-id", default=None, help="Optional stable run directory name.")
...
def main(argv: list[str] | None = None) -> int:
    _add_src_to_path()
    from sok_llm_orchestrator.system_entrypoint import run_text_request

    args = build_parser().parse_args(argv)
    text = " ".join(args.text).strip() if args.text else sys.stdin.read().strip()
    try:
        summary = run_text_request(
            text,
    ...
    if bool(args.fail_on_pipeline_failure) and summary.get("status") != "SUCCEEDED":
        return 2
    return 0
