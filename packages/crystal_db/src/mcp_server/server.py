...
def _tool_novelty_check(args: Dict[str, Any]) -> Dict[str, Any]:
    config = load_config(_as_str(args.get("config_path"), None))
    settings = _resolve_common_settings(args, config)
    db_path = _resolve_db_path(args, config)
    return api.novelty_check(
        db_path,
        structure_id=_as_str(_pick_value(args, config, ["structure_id", "id"], None), None),
        cif_path=_as_str(_pick_value(args, config, ["cif_path", "cif"], None), None),
        k=settings["k"],
        embed_engine=settings["engine"],
        model=settings["model"],
        model_version=settings["model_version"],
        text_sim_threshold=_as_float(_pick_value(args, config, ["text_sim_threshold"], 0.8), 0.8),
        fp_sim_threshold=_as_float(_pick_value(args, config, ["fp_sim_threshold"], 0.95), 0.95),
        force=_as_bool(_pick_value(args, config, ["force"], False), False),
    )


def _tool_bench_retrieval(args: Dict[str, Any]) -> Dict[str, Any]:
    config = load_config(_as_str(args.get("config_path"), None))
    settings = _resolve_common_settings(args, config)
    db_path = _resolve_db_path(args, config)
    cases_path = _as_str(_pick_value(args, config, ["cases_path", "cases"], None), None)
    return api.bench_retrieval(
        db_path,
        cases_path or "",
        k=settings["k"],
        embed_engine=settings["engine"],
...
def _tool_agent(args: Dict[str, Any]) -> Dict[str, Any]:
    config = load_config(_as_str(args.get("config_path"), None))
    db_path = _resolve_db_path(args, config)
    question = _as_str(_pick_value(args, config, ["question", "query"], ""), "") or ""
    return api.agent(
        db_path,
        question=question,
        llm=_as_str(_pick_value(args, config, ["llm"], "off"), "off") or "off",
        run_name=_as_str(_pick_value(args, config, ["run_name"], None), None),
    )
...
TOOL_REGISTRY: Dict[str, ToolSpec] = {
    ...
    "crystal.agent": ToolSpec(
        name="crystal.agent",
        description="Run the Crystal-DB rule-based agent and return agent.v1 output.",
        input_schema={
            "type": "object",
            "properties": {
    ...
    "crystal.novelty_check": ToolSpec(
        name="crystal.novelty_check",
        description="Run novelty-check against Crystal-DB by structure ID or CIF path.",
        input_schema={
            "type": "object",
            "properties": {
                ...
                "model": {"type": "string"},
                "model_version": {"type": "string"},
                "text_sim_threshold": {"type": "number"},
                "fp_sim_threshold": {"type": "number"},
                "force": {"type": "boolean"},
            },
            "additionalProperties": True,
        },
        handler=_tool_novelty_check,
    ),
    ...
    "crystal.bench_retrieval": ToolSpec(
        name="crystal.bench_retrieval",
        description="Run labeled retrieval benchmark and return bench_retrieval.v1 summary.",
        input_schema={
            "type": "object",
            "properties": {
