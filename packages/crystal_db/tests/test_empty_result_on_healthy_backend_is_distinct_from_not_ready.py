import json

from crystal_db.csp_pack import run_csp_pack
from crystal_db.db import connect, init_db


def _seed_single_structure_db(db_path):
    conn = connect(str(db_path))
    init_db(conn)
    ts = "2026-04-09T00:00:00Z"
    sid = "solo-1"
    conn.execute(
        "INSERT INTO structures (structure_id, cif_text, reduced_formula, nsites, volume, license_restricted) "
        "VALUES (?, ?, ?, ?, ?, ?)",
        (sid, f"data_{sid}\n_cell_length_a 1\n", sid, 1, 1.0, 0),
    )
    conn.execute(
        "INSERT INTO metadata (structure_id, formula, elements_csv, space_group, band_gap_eV) "
        "VALUES (?, ?, ?, ?, ?)",
        (sid, sid, ",Ti,O,", "P1", 0.1),
    )
    conn.execute(
        "INSERT INTO provenance (structure_id, source, source_id, retrieved_at, allow_export, allow_derivatives) "
        "VALUES (?, ?, ?, ?, ?, ?)",
        (sid, "test", sid, ts, 1, 1),
    )
    text_doc_id = conn.execute(
        "INSERT INTO text_docs "
        "(structure_id, engine, text_view, engine_version, text, text_sha256, status, error_type, error_message, created_at, updated_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (sid, "caption", "caption", "v1", "single structure text", f"h-{sid}", "OK", None, None, ts, ts),
    ).lastrowid
    conn.execute(
        "INSERT INTO text_embeddings "
        "(text_doc_id, embed_engine, model, model_version, dim, vector, status, error_type, error_message, created_at, updated_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (text_doc_id, "hash", "hash-embed", "v1", 2, json.dumps([1.0, 0.0]), "OK", None, None, ts, ts),
    )
    conn.execute(
        "INSERT INTO structure_fingerprints "
        "(structure_id, fingerprint_method, fingerprint_version, vector_json, feature_names_json, input_hash, generated_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
        (sid, "fp.simple.v1", "v1", json.dumps([1.0, 0.0]), json.dumps(["f1", "f2"]), f"fp-{sid}", ts),
    )
    conn.commit()
    conn.close()


def test_empty_result_on_healthy_backend_is_distinct_from_not_ready(tmp_path):
    healthy_db = tmp_path / "healthy.db"
    missing_db = tmp_path / "missing.db"
    _seed_single_structure_db(healthy_db)

    healthy_empty = run_csp_pack(
        db_path=str(healthy_db),
        query_text=None,
        structure_id="solo-1",
        k=5,
        embed_engine="hash",
        model_name="hash-embed",
        model_version="v1",
        text_engine="caption",
        text_view="caption",
        hybrid=True,
        w_text=0.7,
        w_fp=0.3,
    )
    not_ready = run_csp_pack(
        db_path=str(missing_db),
        query_text="TiO2 rutile startup semantic probe",
        structure_id=None,
        k=5,
        embed_engine="hash",
        model_name="hash-embed",
        model_version="v1",
        text_engine="caption",
        text_view="caption",
        hybrid=True,
        w_text=0.7,
        w_fp=0.3,
    )

    assert healthy_empty["status"] == "empty"
    assert healthy_empty["errors"]["code"] == "empty_result"
    assert healthy_empty["backend_status"]["ready"] is True
    assert healthy_empty["backend_status"]["state"] == "backend_ready"

    assert not_ready["status"] == "error"
    assert not_ready["errors"]["code"] == "missing_db"
    assert not_ready["backend_status"]["ready"] is False
