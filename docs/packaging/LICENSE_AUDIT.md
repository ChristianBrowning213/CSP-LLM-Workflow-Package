...
Christian Browning authorization is not applied to QLIP.

## Excluded software and assets

SCA remains `EXCLUDED_FROM_PUBLIC_RELEASE`. No SCA source, Git dependency,
validation extra, or automatic installer is included. The generic lazy adapter
may be used only with a compatible backend the user is legally entitled to
configure.
